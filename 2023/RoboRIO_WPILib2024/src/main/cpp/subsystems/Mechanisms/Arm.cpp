...
void Arm::Setup() {
	...
	arm_configuration.WithCurrentLimits(ctre::phoenix6::configs::CurrentLimitsConfigs()	
											...
											.WithSupplyCurrentThreshold(RC::kArmMotorContinuousCurrentLimit)
											.WithSupplyTimeThreshold(double(RC::kArmMotorPeakCurrentDurationMs)/1000.0));
...
void Arm::SetExtensionInch(units::inch_t extension_inch, double velocity_factor) {
	...
	if (current_extension < 0.5_in && extension_inch == 0_in && !IsAtZeroLimitSwitch()) {
		...
		m_arm_speed_controller->SetControl(ctre::phoenix6::controls::DutyCycleOut(-0.2, false));
		return;
	}
...
void Arm::ManualDriveArm(double percentage_output) {
	...
	m_arm_speed_controller->SetControl(ctre::phoenix6::controls::DutyCycleOut(percentage_output, false));
}
...
void  Arm::SetupMotionMagic(double velocity_factor) {
	...
	double velocity_rps = velocity_inch_per_second / kArmInchPerRev.value();
	std::cout << "Arm motor cruise speed RPM " << velocity_rps*60 << "\n";
	...
	double acceleration_rpsq = velocity_rps/kArmAccelerationTimeS; 
	...
	m_arm_speed_controller->GetConfigurator().Apply(ctre::phoenix6::configs::MotionMagicConfigs()
														.WithMotionMagicCruiseVelocity(velocity_rps)
														.WithMotionMagicAcceleration(acceleration_rpsq));
}
