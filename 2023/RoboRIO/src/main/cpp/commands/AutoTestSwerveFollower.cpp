...
//                 frc2::CommandPtr(std::unique_ptr<frc2::CommandBase>(new TestTurnCommand(&drivebase))),
//                 frc2::CommandPtr(std::unique_ptr<frc2::CommandBase>(new LogModulesCommand(&drivebase))));
//     }
...
frc2::CommandPtr AutoTestSwerveFollower::CreatePathPlannerCommand(SwerveDrivebase& drivebase, PathType path_type, units::meter_t length,
    ...
    switch (path_type) {
        ...
        default:
            ...
            trajectory = PathPlanner::loadPath(m_path_planner_names[file_path_index], PathConstraints(max_velocity, max_acceleration));
            break;
    }
    ...
    SwerveAutoBuilder pp_swerve_auto_builder(
            ...
            [&drivebase](auto chassis_speeds) { drivebase.Drive(chassis_speeds, false); }, // Output function that accepts robot relative ChassisSpeeds
...
//    if (initial_pose == end_pose && repeat_count > 1) {
        ...
        for (int i = 0; i < repeat_count; i++) {
            ...
            commands.push_back(pp_swerve_auto_builder.followPath(trajectory));
            commands.push_back(frc2::CommandPtr(frc2::InstantCommand([&drivebase, i] {
        ...
        return  frc2::cmd::Sequence(
                ...
                frc2::cmd::Deadline(
                    frc2::cmd::Sequence(
                        pp_swerve_auto_builder.followPath(trajectory),
                        frc2::cmd::Wait(1_s)
                    ),
                    frc2::CommandPtr(std::unique_ptr<frc2::CommandBase>(new LogPathPlannerCommand(&drivebase, trajectory, parameters)))) 
            );
    }
}
...
frc2::CommandPtr AutoTestSwerveFollower::CreateLogDrivebase(SwerveDrivebase& drivebase) {
    return frc2::CommandPtr(std::unique_ptr<frc2::CommandBase>(new LogDrivebaseCommand(&drivebase)));
}
