...
void SwerveDrivebaseShuffleboard::DriveTabSetup() {
    ...
    dt->m_pitch_widget = &drive_tab.Add("Pitch", 0.0).WithPosition(6,0).WithSize(3,2);
    dt->m_roll_widget = &drive_tab.Add("Roll", 0.0).WithPosition(9,0).WithSize(3,2);
...
void SwerveDrivebaseShuffleboard::VisionTabSetup() {
    ...
    vw->m_has_pose = &vision_tab.Add("Has pose", false)
                                ...
                                .WithSize(3,3)
                                .WithWidget(frc::BuiltInWidgets::kBooleanBox);
    vw->m_target_area = &vision_tab.Add("Target Area", 0.0).WithPosition(12,0).WithSize(3,2);
}
...
void SwerveDrivebaseShuffleboard::UpdateShuffleboard() {
    ...
    if (m_vision_widgets != nullptr) {
        frc::SwerveDrivePoseEstimator<4>* estimator = m_drivebase->GetEstimator();
        VisionTabWidgets* vw = m_vision_widgets;

        vw->m_estimator_x->GetEntry()->SetDouble(estimator->GetEstimatedPosition().X().value());
        vw->m_estimator_y->GetEntry()->SetDouble(estimator->GetEstimatedPosition().Y().value());
        vw->m_estimator_heading->GetEntry()->SetDouble(estimator->GetEstimatedPosition().Rotation().Degrees().value());
        ...
        bool has_pose = m_drivebase->GetLatestPhotonPose().has_value();
        vw->m_has_pose->GetEntry()->SetBoolean(has_pose);
        if (has_pose) {
            frc::Pose2d vision_pose = m_drivebase->GetLatestPhotonPose().value().estimatedPose.ToPose2d();
            ...
            vw->m_targets->GetEntry()->SetBoolean(m_drivebase->GetLatestPhotonResult().HasTargets());
            vw->m_target_area->GetEntry()->SetDouble(m_drivebase->GetLatestPhotonResult().GetBestTarget().GetArea());
        } else {
    ...
    if (m_drive_tab_widgets != nullptr) {
        Manipulator::GamePiece game_piece = m_manipulator->GetGamePiece();
        DriveTabWidgets* dt = m_drive_tab_widgets;
