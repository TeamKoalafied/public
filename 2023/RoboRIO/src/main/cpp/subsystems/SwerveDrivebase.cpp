...
void SwerveDrivebase::Periodic() {
    ...
    m_swerve_estimator->Update(m_pigen_imu->GetRotation2d(), module_positions);
...
void SwerveDrivebase::Setup() {
    ...
    m_swerve_estimator = new frc::SwerveDrivePoseEstimator(*m_swerve_drive_kinematics,
                                                            m_pigen_imu->GetRotation2d(), 
                                                                {m_left_front_swerve_module->GetPosition(),
                                                                m_right_front_swerve_module->GetPosition(),
                                                                m_left_back_swerve_module->GetPosition(),
    ...
    frc::SmartDashboard::PutNumber("Slowdown", RC::kDefaultDrivebaseSlowDownFactor);
    ...
    // Create shuffleboard tabs for the swerve modules. Uncomment if needed for testing.
    //m_left_front_swerve_module->CreateShuffleboardDetailedTab("LF");
    //m_right_front_swerve_module->CreateShuffleboardDetailedTab("RF");

    // Create a list of april tags on the field. This will be used for simulation too.
    m_april_tags.push_back({1, frc::Pose3d(2_m, 0_m, 18.25_in, frc::Rotation3d(0_deg, 0_deg, 180_deg))});

    // Create the photon pose estimator
    frc::AprilTagFieldLayout aprilTags{m_april_tags, 54_ft, 27_ft};
    m_photon_estimator = new photonlib::PhotonPoseEstimator(
        aprilTags, photonlib::CLOSEST_TO_REFERENCE_POSE, photonlib::PhotonCamera{"april"}, RC::kRobotToCam);
}
...
void SwerveDrivebase::SimulationInit() {
    ...
    m_sim_vision = new photonlib::SimVisionSystem("april", camFOV, RC::kRobotToCam.Inverse(), led_range,
    ...
    for (frc::AprilTag april_tag : m_april_tags) {
        m_sim_vision->AddSimVisionTarget(photonlib::SimVisionTarget(april_tag.pose, 8_in, 8_in, april_tag.ID));
    }
...
const frc::Pose2d& SwerveDrivebase::GetPose() const {
    return m_swerve_drive_odometry->GetPose();
}
...
units::degree_t SwerveDrivebase::GetPigeonHeading() {
...
void SwerveDrivebase::ResetPose(const frc::Pose2d& pose) {
    ...
    // Reset the pose in the odometry. This requies the current gyro angle
    m_swerve_estimator->ResetPosition(m_pigen_imu->GetRotation2d(),
                                                {m_left_front_swerve_module->GetPosition(),
                                                m_right_front_swerve_module->GetPosition(),
                                                m_left_back_swerve_module->GetPosition(),
                                                m_right_back_swerve_module->GetPosition()},
...
bool SwerveDrivebase::AreWheelsAlignedToDrive(units::meters_per_second_t x_speed, units::meters_per_second_t y_speed, bool field_relative) {
    ...
    units::degree_t fl_delta = units::math::abs(KoalafiedUtilities::NormaliseAngleDiff(fl_state.angle.Degrees(), fl.angle.Degrees()));
    units::degree_t fr_delta = units::math::abs(KoalafiedUtilities::NormaliseAngleDiff(fr_state.angle.Degrees(), fr.angle.Degrees()));
    units::degree_t bl_delta = units::math::abs(KoalafiedUtilities::NormaliseAngleDiff(bl_state.angle.Degrees(), bl.angle.Degrees()));
    units::degree_t br_delta = units::math::abs(KoalafiedUtilities::NormaliseAngleDiff(br_state.angle.Degrees(), br.angle.Degrees()));
 
    if (fl_delta > 90_deg) fl_delta = 180_deg - fl_delta;
    if (fr_delta > 90_deg) fr_delta = 180_deg - fr_delta;
    if (bl_delta > 90_deg) bl_delta = 180_deg - bl_delta;
    if (br_delta > 90_deg) br_delta = 180_deg - br_delta;
...
frc2::CommandPtr SwerveDrivebase::CreateTrajectoryCommand(const frc::Trajectory& trajectory) {
    ...
    return frc2::cmd::Sequence(
        frc2::InstantCommand([this, trajectory] { ResetPose(trajectory.InitialPose()); }, { this }),
        frc2::SwerveControllerCommand<4>(
           trajectory, [this]() { return GetPose(); },
            *m_swerve_drive_kinematics,
            frc2::PIDController{RC::kPXController, 0, 0},
            frc2::PIDController{RC::kPYController, 0, 0}, thetaController,
...
void SwerveDrivebase::DoJoystickControl() {
    ...
    double arm_slowdown = (1.0 - arm_extension_factor) + FULL_EXTENSION_SLOWDOWN * arm_extension_factor;
