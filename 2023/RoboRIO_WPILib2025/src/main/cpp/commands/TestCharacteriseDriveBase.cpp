...
void TestCharacteriseDriveBase::SetupDashboard() {
    ...
    sw->m_voltage = &characterisation_tab.Add("Characterisation Voltage (V)", 3.0).WithPosition(0,3).WithSize(6,3).WithWidget(frc::BuiltInWidgets::kTextView);
    sw->m_distance = &characterisation_tab.Add("Characterisation Distance (m)", 3.0).WithPosition(6,3).WithSize(6,3).WithWidget(frc::BuiltInWidgets::kTextView);
    sw->m_rotations = &characterisation_tab.Add("Characterisation Rotations", 3.0).WithPosition(0,6).WithSize(6,3).WithWidget(frc::BuiltInWidgets::kTextView);
    sw->m_time = &characterisation_tab.Add("Characterisation Time (s)", 3.0).WithPosition(6,6).WithSize(6,3).WithWidget(frc::BuiltInWidgets::kTextView);
}
