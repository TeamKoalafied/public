...
int VisionTestCommand::SetupDashboard(frc::ShuffleboardTab& auto_tab, int x_pos, int y_pos) {
    ...
    sw->m_length = &vison_test_tab.Add("Path Length (m)", 3.0).WithPosition(0, 3).WithSize(4, 2).WithWidget(frc::BuiltInWidgets::kTextView);
    sw->m_width = &vison_test_tab.Add("Path Width (m)", 2.0).WithPosition(0,5).WithSize(4, 2).WithWidget(frc::BuiltInWidgets::kTextView);
    sw->m_rotatation = &vison_test_tab.Add("Path Rotation (deg)", 0.0).WithPosition(0,7).WithSize(4, 2).WithWidget(frc::BuiltInWidgets::kTextView);

    sw->m_max_speed = &vison_test_tab.Add("Max Speed (mps)", 1.0).WithPosition(4,3).WithSize(4, 2).WithWidget(frc::BuiltInWidgets::kTextView);
    sw->m_max_acceleration = &vison_test_tab.Add("Max Accleeration (mps2)", 1.0).WithPosition(4, 5).WithSize(4, 2).WithWidget(frc::BuiltInWidgets::kTextView);
    sw->m_repeat_count = &vison_test_tab.Add("Repeat Count", 1.0).WithPosition(4, 7).WithSize(4, 2).WithWidget(frc::BuiltInWidgets::kTextView);

    sw->m_direction_p = &vison_test_tab.Add("Direction P", 2.0).WithPosition(12, 1).WithSize(4, 2).WithWidget(frc::BuiltInWidgets::kTextView);
    sw->m_direction_d = &vison_test_tab.Add("Direction D", 0.0).WithPosition(12, 3).WithSize(4, 2).WithWidget(frc::BuiltInWidgets::kTextView);
    sw->m_rotation_p = &vison_test_tab.Add("Rotation P", 2.0).WithPosition(12, 5).WithSize(4, 2).WithWidget(frc::BuiltInWidgets::kTextView);
    sw->m_rotation_d = &vison_test_tab.Add("Rotation D", 0.0).WithPosition(12, 7).WithSize(4, 2).WithWidget(frc::BuiltInWidgets::kTextView);
    ...
    if (path_segments[0]) {
        ...
        return  frc2::cmd::Sequence(
                ...
                frc2::cmd::Deadline(
                    ...
                    frc2::CommandPtr(std::unique_ptr<frc2::Command>(new LogVisionCommand(&drivebase, nullptr, parameters)))),
...
std::shared_ptr<PathPlannerPath> VisionTestCommand::GeneratePathSegment(PathConstraints constraints, 
    ...
    std::vector<pathplanner::Waypoint> waypoints = PathPlannerPath::waypointsFromPoses(pose_vector);
    ...
    path = std::make_shared<PathPlannerPath>(
        ...
        std::optional<pathplanner::IdealStartingState>(std::nullopt),
...
void VisionTestCommand::PrintFinalPosition(SwerveDrivebase& drivebase) {
    ...
    if (drivebase.GetVision() != nullptr) {
