...
LogVisionCommand::LogVisionCommand(SwerveDrivebase* drivebase, Manipulator* manipulator, const Parameters parameters) {
...
void LogVisionCommand::LogPose() {
    ...
    const Vision* vision = m_drivebase->GetVision();
    if (vision == nullptr) return;
    
    m_csv_log_file << m_timer.Get().value() << frc::Timer::GetFPGATimestamp().value();
    m_csv_log_file << odo_pose.X().value() << odo_pose.Y().value() << odo_pose.Rotation().Degrees().value();
    m_csv_log_file << vision->GetPose().X().value() << 
                        vision->GetPose().Y().value() << 
                        vision->GetPose().Rotation().Degrees().value();

    // TODO Vision pose and ambiguity need to come from Vision::GetLatestPhotonPose() and/or
    // Vision::GetLatestPhotonResult()
    std::optional<photon::EstimatedRobotPose> raw_vision_pose = vision->GetLatestPhotonPose();
    if (raw_vision_pose.has_value()) {
        ...
        m_csv_log_file << raw_vision_pose.value().estimatedPose.X().value() 
                        ...
                        << angle.value();
        photon::PhotonPipelineResult result = vision->GetLatestPhotonResult();
