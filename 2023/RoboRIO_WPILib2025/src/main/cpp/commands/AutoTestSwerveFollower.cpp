...
int AutoTestSwerveFollower::SetupDashboard(frc::ShuffleboardTab& auto_tab, int x_pos, int y_pos) {
    ...
    sw->m_length = &auto_test_tab.Add("Path Length (m)", 3.0).WithPosition(0, 3).WithSize(4, 2).WithWidget(frc::BuiltInWidgets::kTextView);
    sw->m_width = &auto_test_tab.Add("Path Width (m)", 2.0).WithPosition(0,5).WithSize(4, 2).WithWidget(frc::BuiltInWidgets::kTextView);
    sw->m_rotatation = &auto_test_tab.Add("Path Rotation (deg)", 0.0).WithPosition(0,7).WithSize(4, 2).WithWidget(frc::BuiltInWidgets::kTextView);

    sw->m_max_speed        = &auto_test_tab.Add("Max Speed (mps)", 1.0).WithPosition(4,3).WithSize(4, 2).WithWidget(frc::BuiltInWidgets::kTextView);
    sw->m_max_acceleration = &auto_test_tab.Add("Max Accleeration (mps2)", 1.0).WithPosition(4, 5).WithSize(4, 2).WithWidget(frc::BuiltInWidgets::kTextView);
    sw->m_repeat_count     = &auto_test_tab.Add("Repeat Count", 1.0).WithPosition(4, 7).WithSize(4, 2).WithWidget(frc::BuiltInWidgets::kTextView);

    sw->m_lookahead_distance = &auto_test_tab.Add("Look ahead distance (m)", 0.5).WithPosition(8,3).WithSize(4, 2).WithWidget(frc::BuiltInWidgets::kTextView);
    sw->m_lookahead_factor = &auto_test_tab.Add("Lookahead factor", 1.5).WithPosition(8,5).WithSize(4, 2).WithWidget(frc::BuiltInWidgets::kTextView);
    sw->m_correction_blend = &auto_test_tab.Add("Correction blend", 0.5).WithPosition(8,7).WithSize(4, 2).WithWidget(frc::BuiltInWidgets::kTextView);

    sw->m_direction_p = &auto_test_tab.Add("Direction P", 2.0).WithPosition(12, 1).WithSize(4, 2).WithWidget(frc::BuiltInWidgets::kTextView);
    sw->m_direction_d = &auto_test_tab.Add("Direction D", 0.0).WithPosition(12, 3).WithSize(4, 2).WithWidget(frc::BuiltInWidgets::kTextView);
    sw->m_rotation_p = &auto_test_tab.Add("Rotation P", 2.0).WithPosition(12, 5).WithSize(4, 2).WithWidget(frc::BuiltInWidgets::kTextView);
    sw->m_rotation_d = &auto_test_tab.Add("Rotation D", 0.0).WithPosition(12, 7).WithSize(4, 2).WithWidget(frc::BuiltInWidgets::kTextView);
...
frc2::CommandPtr AutoTestSwerveFollower::CreatePathPlannerCommand(SwerveDrivebase& drivebase, PathType path_type, units::meter_t length,
    ...
    frc::Pose2d initial_pose(trajectory.get()->getStartingHolonomicPose().value()); 
    if (repeat_count > 1) {
