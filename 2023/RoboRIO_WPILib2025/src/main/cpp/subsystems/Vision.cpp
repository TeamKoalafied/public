...
//        aprilTags, photon::AVERAGE_BEST_TARGETS, photon::PhotonCamera{"april"}, RC::kRobotToCam);
        aprilTags, photon::CLOSEST_TO_REFERENCE_POSE, RC::kRobotToCam);
...
void Vision::Update(wpi::array<double, 3> std_devs) {
    ...
    // Update the vision
    m_latest_photon_result = m_camera.GetLatestResult();
    m_latest_photon_pose = m_photon_estimator->Update(m_latest_photon_result);
...
void Vision::SetupSimulation() {
    ...
    m_sim_vision = new photon::VisionSystemSim("april");
    m_sim_vision->AddCamera(&m_april_sim,RC::kRobotToCam);
                // , camFOV, .Inverse(), led_range,
    ...
    for (frc::AprilTag april_tag : m_april_tags) {
        m_sim_vision->AddVisionTargets(std::vector<photon::VisionTargetSim>{photon::VisionTargetSim(april_tag.pose, photon::TargetModel(8_in, 8_in), april_tag.ID)});
    }
}

void Vision::UpdateSimulation() {
    m_sim_vision->Update(GetPose());
}
