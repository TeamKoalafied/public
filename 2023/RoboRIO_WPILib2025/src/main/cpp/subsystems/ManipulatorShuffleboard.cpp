...
void ManipulatorShuffleboard::SetupArm(frc::ShuffleboardTab& tab, int x, int y) {
    ...
    aw->m_current_widget = &layout.Add("Arm Current", 0.0).WithPosition(2, 0).WithSize(3, 2).WithWidget(frc::BuiltInWidgets::kTextView);
	aw->m_extension_widget = &layout.Add("Arm extension",0.0).WithPosition(3,0).WithSize(3,2).WithWidget(frc::BuiltInWidgets::kTextView);
}
...
void ManipulatorShuffleboard::SetupIntake(frc::ShuffleboardTab& tab, int x, int y) {
	...
	// Add the widgets to the layout
    iw->m_roller_current_widget = &layout.Add("Roller Current", 0.0).WithPosition(4, 0).WithSize(3, 2).WithWidget(frc::BuiltInWidgets::kTextView);
}
...
void ManipulatorShuffleboard::SetupPivot(frc::ShuffleboardTab& tab, int x, int y) {
    ...
    pw->m_current_widget = &layout.Add("Pivot Current", 0.0).WithPosition(2, 0).WithSize(3, 2).WithWidget(frc::BuiltInWidgets::kTextView);
	pw->m_angle_widget = &layout.Add("Pivot Angle",0.0).WithPosition(3,0).WithSize(3,2).WithWidget(frc::BuiltInWidgets::kTextView);
}
...
void ManipulatorShuffleboard::SetupWrist(frc::ShuffleboardTab& tab, int x, int y) {
    ...
    ww->m_current_widget = &layout.Add("Wrist Current", 0.0).WithPosition(2, 0).WithSize(3, 2).WithWidget(frc::BuiltInWidgets::kTextView);
	ww->m_angle_widget = &layout.Add("Wrist Angle",0.0).WithPosition(3,0).WithSize(3,2).WithWidget(frc::BuiltInWidgets::kTextView);
}
...
void ManipulatorShuffleboard::SetupTuneIntake(frc::ShuffleboardTab& tab, int x, int y) {
	...
	// Add the widgets to the layout
    itw->m_cone_speed = &layout.Add("Cone speed", DEFAULT_CONE_SPEED).WithPosition(0,0).WithSize(3,2).WithWidget(frc::BuiltInWidgets::kTextView);
    itw->m_cube_speed = &layout.Add("Cube speed", DEFAULT_CUBE_SPEED).WithPosition(0,2).WithSize(3,2).WithWidget(frc::BuiltInWidgets::kTextView);
    itw->m_cone_current = &layout.Add("Cone current", DEFAULT_CONE_CURRENT).WithPosition(3,0).WithSize(3,2).WithWidget(frc::BuiltInWidgets::kTextView);
    itw->m_cube_current = &layout.Add("Cube current", DEFAULT_CUBE_CURRENT).WithPosition(3,2).WithSize(3,2).WithWidget(frc::BuiltInWidgets::kTextView);  
    itw->m_cone_timeout = &layout.Add("Cone timeout", DEFAULT_CONE_TIMEOUT).WithPosition(6,0).WithSize(3,2).WithWidget(frc::BuiltInWidgets::kTextView);
    itw->m_cube_timeout = &layout.Add("Cube timeout", DEFAULT_CUBE_TIMEOUT).WithPosition(6,2).WithSize(3,2).WithWidget(frc::BuiltInWidgets::kTextView); 
}
...
void ManipulatorShuffleboard::SetupTunePosition(frc::ShuffleboardTab& tab, int x, int y) {
	...
	// Add the widgets to the layout
    tpw->m_pivot_angle_widget = &layout.Add("Pivot Angle", 0.0).WithPosition(0, 0).WithSize(3, 2).WithWidget(frc::BuiltInWidgets::kTextView);
    tpw->m_arm_extension_widget = &layout.Add("Arm Extension", 0.0).WithPosition(1, 0).WithSize(3, 2).WithWidget(frc::BuiltInWidgets::kTextView);
    tpw->m_wrist_angle_widget = &layout.Add("Wrist Angle", 0.0).WithPosition(2, 0).WithSize(3, 2).WithWidget(frc::BuiltInWidgets::kTextView);
}
