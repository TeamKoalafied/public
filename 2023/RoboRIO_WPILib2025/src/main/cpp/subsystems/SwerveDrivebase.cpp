...
SwerveDrivebase::SwerveDrivebase(Manipulator* manipulator) :
    m_joystick_timer { "DB Joystick", true },
    m_simulation_timer { "DB Simulation", true },
    m_periodic_timer { "DB Periodic", true }
{
...
void SwerveDrivebase::Setup() {
    ...
    if (m_left_front_swerve_module->GetCanError() || m_right_front_swerve_module->GetCanError() ||
        m_left_back_swerve_module->GetCanError() || m_right_back_swerve_module->GetCanError()) {
        m_drivebase_ok = false;
    }
...
void SwerveDrivebase::DoJoystickControl() {
    ...
    if (x_speed == 0_mps && y_speed == 0_mps && rotate_speed == 0_rad_per_s) {
        ...
        if (m_controller->GetRightBumperButton()) slow_speed *= 4.0;
    ...
    bool field_relative = !m_controller->GetRightBumperButton();
...
void SwerveDrivebase::DoTuningJoystickControl() {
    ...
    double closed_loop = m_controller->GetLeftBumperButton();
    double max_rmp = 5000;
    ...
    closed_loop = m_controller->GetRightBumperButton();
