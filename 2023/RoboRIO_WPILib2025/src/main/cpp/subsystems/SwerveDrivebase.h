...
public:
    ...
    // Get the odometry object for tracking the drivebase position on the field
    ...
    // Get the vision manager. Can be nullptr when testing.
    const Vision* GetVision() const { return m_vision; }

    // Flag for indicating if the drivebase is working correctly
    bool GetDrivebaseOK() const { return m_drivebase_ok; }
    ...
    frc::XboxController* GetController() const { return m_controller; }
...
private:
    ...
    Vision* m_vision = nullptr;                             // Vision manager. Can be nullptr when testing.
    ...
    bool m_drivebase_ok = true;                             // Flag for indicating if the drivebase is working correctly
    ...
    DurationTimer m_joystick_timer { "DB Joystick" };
    DurationTimer m_simulation_timer { "DB Simulation" };
    DurationTimer m_periodic_timer { "DB Periodic" };
