...
void Intake::Setup() {
    ...
    m_intake_speed_controller = new TalonSRX(RC::kIntakeTalonId);
    ...
    // Current limit
    winch_configuration.continuousCurrentLimit = RC::kWinchMotorContinuousCurrentLimit.value();
    winch_configuration.peakCurrentLimit = RC::kWinchMotorPeakCurrentLimit.value();
    winch_configuration.peakCurrentDuration = RC::kWinchMotorPeakCurrentDurationMs.convert<units::millisecond>().value();
...
void Intake::CreateShuffleboardSummary(frc::ShuffleboardTab& shuffleboard_tab, int x, int y) {
    ...
    iw->m_winch_position_widget = &layout.Add("Winch Position", 0.0).WithPosition(0,0).WithSize(3,2).WithWidget(frc::BuiltInWidgets::kTextView);
    iw->m_winch_current_widget = &layout.Add("Winch Current", 0.0).WithPosition(1,0).WithSize(3,2).WithWidget(frc::BuiltInWidgets::kTextView);
    iw->m_winch_state_widget = &layout.Add("Winch State", "Not Set").WithPosition(2, 0).WithSize(3, 2).WithWidget(frc::BuiltInWidgets::kTextView);
    iw->m_winch_encoder_widget = &layout.Add("Winch Encoder", 0.0).WithPosition(3, 0).WithSize(3, 2).WithWidget(frc::BuiltInWidgets::kTextView);
