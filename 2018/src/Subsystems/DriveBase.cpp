...
#include "Commands/DrivePathfinder.h"
#include "Commands/TestCharacteriseDriveBase.h"
...
void DriveBase::Periodic() {
    ...
    //double distance_left = m_left_master_speed_controller->GetSelectedSensorPosition(0)*4*25.4*3.1415/4096.0;
...
//    m_right_master_speed_controller->ConfigVelocityMeasurementWindow(int windowSize, kTalonTimeoutMs);
    ...
    m_left_master_speed_controller->ConfigNominalOutputForward(RobotConfiguration::kDriveMotorNominalOutput, kTalonTimeoutMs);
    m_left_master_speed_controller->ConfigNominalOutputReverse(-RobotConfiguration::kDriveMotorNominalOutput, kTalonTimeoutMs);
    m_left_master_speed_controller->ConfigPeakOutputForward(+1.0f, kTalonTimeoutMs);
    m_left_master_speed_controller->ConfigPeakOutputReverse(-1.0f, kTalonTimeoutMs);
    m_right_master_speed_controller->ConfigNominalOutputForward(RobotConfiguration::kDriveMotorNominalOutput, kTalonTimeoutMs);
    m_right_master_speed_controller->ConfigNominalOutputReverse(-RobotConfiguration::kDriveMotorNominalOutput, kTalonTimeoutMs);
    m_right_master_speed_controller->ConfigPeakOutputForward(+1.0f, kTalonTimeoutMs);
    m_right_master_speed_controller->ConfigPeakOutputReverse(-1.0f, kTalonTimeoutMs);

    // Set the ramp rate for open and close loop modes. TODO Is this necessary for slaves?
    // The close loop is used for driver control, so use a larger value to make it easier to control
    // The open loop is used for autonomous, so use a small or zero value so that the ramp has not
    // interfere with the autonomous control loop.
    // TSSRM Section 6 (page 41)
    m_left_master_speed_controller->ConfigOpenloopRamp(RobotConfiguration::kDriveMotorRampRateAutonomousS, kTalonTimeoutMs);
    m_left_slave_speed_controller->ConfigOpenloopRamp(RobotConfiguration::kDriveMotorRampRateAutonomousS, kTalonTimeoutMs);
    m_left_slave2_speed_controller->ConfigOpenloopRamp(RobotConfiguration::kDriveMotorRampRateAutonomousS, kTalonTimeoutMs);
    m_right_master_speed_controller->ConfigOpenloopRamp(RobotConfiguration::kDriveMotorRampRateAutonomousS, kTalonTimeoutMs);
    m_right_slave_speed_controller->ConfigOpenloopRamp(RobotConfiguration::kDriveMotorRampRateAutonomousS, kTalonTimeoutMs);
    m_right_slave2_speed_controller->ConfigOpenloopRamp(RobotConfiguration::kDriveMotorRampRateAutonomousS, kTalonTimeoutMs);
    m_left_master_speed_controller->ConfigClosedloopRamp(RobotConfiguration::kDriveMotorRampRateDriverS, kTalonTimeoutMs);
    m_left_slave_speed_controller->ConfigClosedloopRamp(RobotConfiguration::kDriveMotorRampRateDriverS, kTalonTimeoutMs);
    m_left_slave2_speed_controller->ConfigClosedloopRamp(RobotConfiguration::kDriveMotorRampRateDriverS, kTalonTimeoutMs);
    m_right_master_speed_controller->ConfigClosedloopRamp(RobotConfiguration::kDriveMotorRampRateDriverS, kTalonTimeoutMs);
    m_right_slave_speed_controller->ConfigClosedloopRamp(RobotConfiguration::kDriveMotorRampRateDriverS, kTalonTimeoutMs);
    m_right_slave2_speed_controller->ConfigClosedloopRamp(RobotConfiguration::kDriveMotorRampRateDriverS, kTalonTimeoutMs);
...
//	virtual void EnableVoltageCompensation(bool enable);
    ...
    m_left_master_speed_controller->ConfigContinuousCurrentLimit(RobotConfiguration::kDriveMotorContinuousCurrentLimitA, kTalonTimeoutMs);
    m_left_master_speed_controller->ConfigPeakCurrentLimit(RobotConfiguration::kDriveMotorPeakCurrentLimitA, kTalonTimeoutMs);
    m_left_master_speed_controller->ConfigPeakCurrentDuration(RobotConfiguration::kDriveMotorPeakCurrentDurationMs, kTalonTimeoutMs);
    m_left_master_speed_controller->EnableCurrentLimit(true);
    m_left_slave_speed_controller->ConfigContinuousCurrentLimit(RobotConfiguration::kDriveMotorContinuousCurrentLimitA, kTalonTimeoutMs);
    m_left_slave_speed_controller->ConfigPeakCurrentLimit(RobotConfiguration::kDriveMotorPeakCurrentLimitA, kTalonTimeoutMs);
    m_left_slave_speed_controller->ConfigPeakCurrentDuration(RobotConfiguration::kDriveMotorPeakCurrentDurationMs, kTalonTimeoutMs);
    m_left_slave_speed_controller->EnableCurrentLimit(true);
    m_left_slave2_speed_controller->ConfigContinuousCurrentLimit(RobotConfiguration::kDriveMotorContinuousCurrentLimitA, kTalonTimeoutMs);
    m_left_slave2_speed_controller->ConfigPeakCurrentLimit(RobotConfiguration::kDriveMotorPeakCurrentLimitA, kTalonTimeoutMs);
    m_left_slave2_speed_controller->ConfigPeakCurrentDuration(RobotConfiguration::kDriveMotorPeakCurrentDurationMs, kTalonTimeoutMs);
    m_left_slave2_speed_controller->EnableCurrentLimit(true);
    m_right_master_speed_controller->ConfigContinuousCurrentLimit(RobotConfiguration::kDriveMotorContinuousCurrentLimitA, kTalonTimeoutMs);
    m_right_master_speed_controller->ConfigPeakCurrentLimit(RobotConfiguration::kDriveMotorPeakCurrentLimitA, kTalonTimeoutMs);
    m_right_master_speed_controller->ConfigPeakCurrentDuration(RobotConfiguration::kDriveMotorPeakCurrentDurationMs, kTalonTimeoutMs);
    m_right_master_speed_controller->EnableCurrentLimit(true);
    m_right_slave_speed_controller->ConfigContinuousCurrentLimit(RobotConfiguration::kDriveMotorContinuousCurrentLimitA, kTalonTimeoutMs);
    m_right_slave_speed_controller->ConfigPeakCurrentLimit(RobotConfiguration::kDriveMotorPeakCurrentLimitA, kTalonTimeoutMs);
    m_right_slave_speed_controller->ConfigPeakCurrentDuration(RobotConfiguration::kDriveMotorPeakCurrentDurationMs, kTalonTimeoutMs);
    m_right_slave_speed_controller->EnableCurrentLimit(true);
    m_right_slave2_speed_controller->ConfigContinuousCurrentLimit(RobotConfiguration::kDriveMotorContinuousCurrentLimitA, kTalonTimeoutMs);
    m_right_slave2_speed_controller->ConfigPeakCurrentLimit(RobotConfiguration::kDriveMotorPeakCurrentLimitA, kTalonTimeoutMs);
    m_right_slave2_speed_controller->ConfigPeakCurrentDuration(RobotConfiguration::kDriveMotorPeakCurrentDurationMs, kTalonTimeoutMs);
    ...
    m_right_master_speed_controller->Config_kF(kRunProfileSlotIdx, RobotConfiguration::kDriveBasePidF, kTalonTimeoutMs);
    m_right_master_speed_controller->Config_kP(kRunProfileSlotIdx, RobotConfiguration::kDriveBasePidP, kTalonTimeoutMs);
    m_right_master_speed_controller->Config_kI(kRunProfileSlotIdx, RobotConfiguration::kDriveBasePidI, kTalonTimeoutMs);
    m_right_master_speed_controller->Config_kD(kRunProfileSlotIdx, RobotConfiguration::kDriveBasePidD, kTalonTimeoutMs);
...
//			int timeoutMs);
    ...
    // Setup some driving commands that will be triggered by joystick buttons
    // m_forward_command = new DriveStraight(2.0, 24.0, m_pigen_imu->GetFusedHeading());
    m_forward_command = new DrivePath(DrivePath::PathStrategy::kSwitchLeft);
    m_backward_command = new DriveStraight(-2.0, -24.0, m_pigen_imu->GetFusedHeading());
    m_left_command = new DriveRotate(-90.0);
    m_right_command = new DriveRotate(90.0);
    m_180_command = new DriveRotate(180.0);
    m_rotate_pid_command = new DriveRotatePID(DriveRotatePID::AngleType::kRelative, 90.0);

//    m_find_cube_command = new VisionFindCube();

    SmartDashboard::PutNumber("Velocity", 5.0);
    SmartDashboard::PutNumber("Distance", 72.0);
    SmartDashboard::PutNumber("MoveLimit", 0.5);
    SmartDashboard::PutNumber("Straight Gain", 0.01);
...
//	return;
    ...
    SmartDashboard::PutNumber("ARotate", rotate);
    ...
    // Do joystick control of drivebase characterisation. Comment this out unless you are actually
    // doing drivebase characterisation.
    // TestCharacteriseDriveBase::DoJoystickControl(m_joystick);

	// Do joystick control of following Pathfinder paths for testing. Comment this out unless you
    // are actually doing Pathfinder testing.
    DrivePathfinder::DoJoystickTestControl(m_joystick);
}
...
void DriveBase::GetMovementFromJoystick(double& move, double& rotate) {
    ...
    // acceleration).
    move   = PowerAdjust(move, RobotConfiguration::kMovePower);
    rotate = PowerAdjust(rotate, RobotConfiguration::kRotatePower);
...
void DriveBase::ArcadeDrive(double move_value, double rotate_value) {
    ...
    SmartDashboard::PutNumber("LeftSet", left_motor_velocity_native);
    SmartDashboard::PutNumber("RightSet", right_motor_velocity_native);

}


void DriveBase::CalculateDriveStraightAdjustment(double move, double& rotate) {
    SmartDashboard::PutNumber("Heading", GetPigeonHeading());
