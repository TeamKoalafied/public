...
bool AutonomousCommand::SetupCommand() {
	...
	result = SmartDashboard::GetString("Auto Selector", "");
	...
	std::cout << "Alliance: ";
	switch (alliance) {
		case DriverStation::kRed:     std::cout << "Red\n"; break;
		case DriverStation::kBlue:    std::cout << "Blue\n"; break;
		case DriverStation::kInvalid: std::cout << "Invalid\n"; break;
	}
	...
	switch (m_strategy) {
		...
		case Strategy::kPathfinderSwitch:   InitialiseLoadSwitchPathfinder();   break;
...
void AutonomousCommand::SetupDashboard() { // static
	...
	//m_stratgey_chooser.
	m_stratgey_chooser.AddObject("Drive Forward", Strategy::kDriveForward);
	m_stratgey_chooser.AddDefault("Load Switch", Strategy::kLoadSwitch);
	m_stratgey_chooser.AddObject("Load Switch Simple Path", Strategy::kPathSwitch);
	m_stratgey_chooser.AddObject("Load Switch PathFinder", Strategy::kPathfinderSwitch);
	m_stratgey_chooser.AddObject("Load Scale", Strategy::kLoadScale);
	m_stratgey_chooser.AddObject("Load Scale Simple Path", Strategy::kPathScale);
	m_stratgey_chooser.AddObject("Test Drive and Return", Strategy::kTestDriveAndReturn);

	SmartDashboard::PutData("Autonomous Strategy", &m_stratgey_chooser);

	m_position_chooser.AddObject("Left", Position::kLeft);
	m_position_chooser.AddObject("Right", Position::kRight);
	m_position_chooser.AddDefault("Middle", Position::kMiddle);
	SmartDashboard::PutData("Starting Position", &m_position_chooser);
...
//	SmartDashboard::init();
	SmartDashboard::PutStringArray("Auto List", auto_mode_labels);
	SmartDashboard::UpdateValues();
...
void AutonomousCommand::EchoSettingsToDashboard() { // static
	...
	SmartDashboard::PutString("Auto Settings", message);

	// Echo the default dashboard selection back to the default dashboard as a sanity check
	SmartDashboard::PutString("DB/String 0", result);
}
...
{
// HACKED Removed as DrivePathfinder has change. This code was not being used anyway.
