...
public:
    ...
    frc::SwerveModuleState GetDesiredState() const;
    ...
    //
    // state - desired state of the module to target
    // dry_steer_allowed - whether the module is allowed to dry steer (change angle without driving)
    ...
    // Get current value of the scrub metric (difference between desired and actual angle of the module)
    units::degree_t GetScrub();

    // Set the value of the scrub metric (done by the drivebase which knows the desired state)
    void SetScrub(units::degree_t scrub);
    ...
    //
    // drive_fraction - percentage output for the drive motor
    // steer_fraction - percentage output for the steer motor
    void ManualDriveMotors(double drive_fraction, double steer_fraction);

    // Set the current state (speed & angle) of this module for characterisation
    //
    // angle - Angle to set the module to
    // drive_fraction - Faction of maximum voltage to apply to the drive motor [0, 1]
...
private:
    ...
    mutable ctre::phoenix6::hardware::TalonFX m_drive_controller;         // Talon FX drive speed controller
    mutable ctre::phoenix6::hardware::TalonFX m_steer_controller;         // Talon FX steer speed controller
    ctre::phoenix6::hardware::CANcoder* m_steer_encoder;          // CAN id of the CANCoder encoder for the steering wheel
    ...
    mutable frc::SimpleWidget* m_scrub_widget;

    units::degree_t m_scrub_metric = 0_deg;

    Logging::CsvFile* m_csv_log_file = nullptr;             // CSV file for logging debugging stuff
    units::degree_t m_target_steer_motor_angle = 0_deg;     // Target stear motor angle saved for debugging
    units::revolutions_per_minute_t m_target_drive_motor_rpm;   // Target drive motor speed in RM saved for debuggin
};
