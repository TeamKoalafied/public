...
int AutoTestSwerveFollower::SetupDashboard(frc::ShuffleboardTab& auto_tab, int x_pos, int y_pos) {
    ...
    GetPathPlannerAutoFilePaths(m_path_planner_names);
    for (int i = 0; i < (int)m_path_planner_names.size(); i++) {
        m_shuffleboard_widgets->m_path_chooser.AddOption("File- " + m_path_planner_names[i], (PathType)((int)PathType::FilePath + i));
    }
...
frc2::CommandPtr AutoTestSwerveFollower::CreatePathPlannerCommand(SwerveDrivebase& drivebase, PathType path_type, units::meter_t length,
    ...
    PathPlanner::SetupSwerveAuto(drivebase, nullptr, parameters.m_direction_p, parameters.m_direction_d, parameters.m_rotation_p, parameters.m_rotation_d);
    ...
    switch (path_type) {
        case PathType::Forward: {
            ...
            std::vector<frc::Pose2d> poses{
                ...
                frc::Pose2d(length, 0.0_m, frc::Rotation2d(0_deg))
            };
        ...
        case PathType::Turn: {
            std::vector<frc::Pose2d> poses{
                frc::Pose2d(0_m, 0_m, frc::Rotation2d(0_deg)),
                frc::Pose2d(length, width, frc::Rotation2d(rotation))
            };
            std::vector<frc::Translation2d> bezierPoints = PathPlannerPath::bezierFromPoses(poses);
        ...
        default:
            ...
            // try {
            //     path = PathPlannerPath::fromPathFile(m_path_planner_names[file_path_index]);
            // } catch (const std::exception& e) {
            //     std::cout << "Exception loading file " << e.what() << "\n";
            //     return frc2::CommandPtr(frc2::InstantCommand([] {  }, {  }));
            // }

            // Otherwise it must be one of the auto files, so calculate the index and load it
            int file_path_index = (int)path_type - (int)PathType::FilePath;
            if (file_path_index < 0 || file_path_index >= (int)m_path_planner_names.size()) {
                std::cout << "ERROR: Illegal path file index\n";
                return frc2::CommandPtr(frc2::InstantCommand([] {  }, {  }));
            ...
            std::cout << "Loading path file " << m_path_planner_names[file_path_index] << "\n";
            auto_file_path = m_path_planner_names[file_path_index];
            break;
    }
...
//    if (initial_pose == end_pose && repeat_count > 1) {
        ...
        frc::Pose2d initial_pose(path.get()->getPreviewStartingHolonomicPose()); 
        ...
        for (int i = 0; i < repeat_count; i++) {
            ...
            commands.push_back(AutoBuilder::followPath(path));
            commands.push_back(frc2::CommandPtr(frc2::InstantCommand([&drivebase, i] {
        ...
        return  frc2::cmd::Sequence(
                ...
                frc2::cmd::Deadline(
                    frc2::cmd::Sequence(
//                        autonomous_command.value(),
                        AutoBuilder::followPath(path),
                        frc2::cmd::Wait(1_s)
                    ),
                    frc2::CommandPtr(std::unique_ptr<frc2::Command>(new LogPathPlannerCommand(&drivebase, nullptr, path, parameters)))) 
            );
    } else {
        std::vector<std::shared_ptr<PathPlannerPath>> auto_paths = PathPlannerAuto::getPathGroupFromAutoFile(auto_file_path);

        return  frc2::cmd::Sequence(
                //frc2::CommandPtr(frc2::InstantCommand([&drivebase, initial_pose] { drivebase.ResetPose(initial_pose); }, { &drivebase })),
                frc2::cmd::Deadline(
                    frc2::cmd::Sequence(
                        PathPlannerAuto(auto_file_path).ToPtr(),
                        frc2::cmd::Wait(1_s)
                    ),
                    frc2::CommandPtr(std::unique_ptr<frc2::Command>(new LogPathPlannerCommand(&drivebase, nullptr, auto_paths, parameters)))) 
            );

    }
}

void AutoTestSwerveFollower::GetPathPlannerPathFilePaths(std::vector<std::string>& path_names) {

    std::filesystem::path pathfinder_dir(frc::filesystem::GetDeployDirectory());
    pathfinder_dir /= "pathplanner/paths";
...
void AutoTestSwerveFollower::GetPathPlannerAutoFilePaths(std::vector<std::string>& path_names) {

    std::filesystem::path pathfinder_dir(frc::filesystem::GetDeployDirectory());
    pathfinder_dir /= "pathplanner/autos";
...
frc2::CommandPtr AutoTestSwerveFollower::CreateLogDrivebase(SwerveDrivebase& drivebase) {
    return frc2::CommandPtr(std::unique_ptr<frc2::Command>(new LogDrivebaseCommand(&drivebase, nullptr)));
}
