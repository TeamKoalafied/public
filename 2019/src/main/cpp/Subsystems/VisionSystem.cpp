...
void VisionSystem::VisionThread() {
	...
	cs::CvSource outputStream = CameraServer::GetInstance()->PutVideo("Live", kWidth, kHeight);

	// Setup a CvSource. This will send processed images back to the Dashboard
	cs::CvSource outputStream2 = CameraServer::GetInstance()->PutVideo("Processed", kWidth, kHeight);
