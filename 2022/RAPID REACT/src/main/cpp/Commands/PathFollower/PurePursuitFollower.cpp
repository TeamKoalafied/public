...
//				mechanism_action.m_time_period = total_time_periods + mechanism_action.m_time / TIME_PERIOD;
    ...
    std::cout << "Starting Segment " << GetPathSegment().m_name << " DONE " << m_path_points.size() << "\n";
...
void PurePursuitFollower::FollowSegment() {
	...
	Sample sample;
	sample.m_time_s = m_timer.Get().value();
    ...
    if (path_segment.m_path_definition.empty()) {
        ...
        // DEBUG
        //std::cout << "FollowSegment no points doing actions\n";
    } else {
        ...
        // DEBUG
        //std::cout << "FollowSegment Left " << left_output << " Right " << right_output << " Finished " << m_segment_finished << "\n";
    }
