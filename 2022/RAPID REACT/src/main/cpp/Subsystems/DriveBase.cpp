...
void DriveBase::TeleopInit() {
    // Do not reset the position when Teleop starts as we do not want this to happen
    // in a match
//    ResetPosition();
}

void DriveBase::DisabledInit() {
    // Reset the drivebase position so that tracking can work when practicing and debugging
    //ResetPosition();
...
void DriveBase::DoCheezyDrive() {
    ...
    // Apply a slowdown factor to all movement. This defaults to 1.0 and that is
    // used for matches. Can be set less for testing or demos.
    double slowdown_fractor = frc::SmartDashboard::GetNumber("Slowdown", RC::kDefaultDrivebaseSlowDownFactor);
    slowdown_fractor = std::clamp(slowdown_fractor, 0.1, 1.0);
    move *= slowdown_fractor;
    rotate *= slowdown_fractor;
